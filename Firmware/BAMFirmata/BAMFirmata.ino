...
#define JOYSTIC_UP 8
#define JOYSTIC_DOWN 6
#define JOYSTIC_RIGHT 9
#define JOYSTIC_LEFT 5

#define PEN_PIN 14

#define LIMIT_SWITCH 19
        ...
        if (stepCommand == STEPPER_CONFIG)
        {
          ...
          if (interfaceType == Stepper::DRIVER || interfaceType == Stepper::TWO_WIRE)
          {
            if (argc > 7) {
              ...
              if (limitSwitch1 > 0)
              {
                setPinModeCallback(limitSwitch1, l1usePullup ? FIRMATA_INPUT_PULLUP : INPUT);
              }
              if (limitSwitch2 > 0)
              {
                setPinModeCallback(limitSwitch2, l2usePullup ? FIRMATA_INPUT_PULLUP : INPUT);
              }
...
void setup()
{
  ...
  if(digitalRead(10) == LOW){
    ...
    pinMode(JOYSTIC_UP, INPUT_PULLUP);
    pinMode(JOYSTIC_DOWN, INPUT_PULLUP);
    pinMode(JOYSTIC_LEFT, INPUT_PULLUP);
    pinMode(JOYSTIC_RIGHT, INPUT_PULLUP);
...
void loop()
{
  ...
  if (currentMillis - previousMillis > samplingInterval)
  {
    ...
    //   wdt_reset();
    digitalRead(GO_BUT)==LOW? digitalWrite(PEN_PIN, HIGH):digitalWrite(PEN_PIN, LOW);

    int vertical = 0;
    int horizontal = 0;

    digitalRead(JOYSTIC_UP)==LOW? vertical=1 : 0;
    digitalRead(JOYSTIC_DOWN)==LOW? vertical=-1 : 0;
    digitalRead(JOYSTIC_LEFT)==LOW? horizontal=1 : 0;
    digitalRead(JOYSTIC_RIGHT)==LOW? horizontal=-1 : 0;

    if(digitalRead(LIMIT_SWITCH2)==LOW && digitalRead(JOYSTIC_RIGHT)==LOW && horizontal<0)
      horizontal=0;
    if(digitalRead(LIMIT_SWITCH)==LOW && digitalRead(JOYSTIC_LEFT)==LOW && horizontal>0)
      horizontal=0;

    float usDelay = 70;
    vertical>0? digitalWrite(DIR_PIN2, HIGH): digitalWrite(DIR_PIN2, LOW);
    horizontal>0? digitalWrite(DIR_PIN, HIGH): digitalWrite(DIR_PIN, LOW);
